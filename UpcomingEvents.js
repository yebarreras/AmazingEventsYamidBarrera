document.addEventListener("DOMContentLoaded", () => {
    ...
    let eventosFuturos = data.events.filter(evento => new Date(data.currentDate) < new Date(evento.date));
            ...
            checkbox.addEventListener("change", function (event) {
                if (event.target.checked) {
                    if (tarjetasBuscadas.length !== 0) {
                        ...
                        eventosCheckeados = eventosFuturos.filter(evento => checkboxCheckeados.includes(evento.category));
                        containerCards.innerHTML = "";
    ...
    search.addEventListener("keyup", (event) => {
        if (eventosCheckeados.length !== 0) {
            ...
            let texto = event.target.value.toLowerCase();
            let tarjetasBuscadas = filtrarTexto(eventosFuturos, texto);
            containerCards.innerHTML = "";
            createCards(tarjetasBuscadas);
            if (tarjetasBuscadas.length === 0) {
                containerCards.innerHTML = "<h3>No hay resultados</h3>"
