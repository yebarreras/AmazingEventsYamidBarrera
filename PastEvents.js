document.addEventListener("DOMContentLoaded", () => {
    ...
    let eventosPasados = data.events.filter(evento => new Date(data.currentDate) > new Date(evento.date));
    ...
    function pintarCheckbox(arreglo) {
        ...
        for (let i = 0; i < categorias.length; i++) {
            ...
            checkbox.className = "form-check form-check-inline m-1"
            checkbox.value = categorias[i]
            checkbox.id = categorias[i]
            let label = document.createElement('label')
            label.className = 'form-check-label me-3 mt-1'
            ...
            checkbox.addEventListener("change", function (event) {
                if (event.target.checked) {
                    if (tarjetasBuscadas.length !== 0) {
                        ...
                        eventosCheckeados = eventosPasados.filter(evento => checkboxCheckeados.includes(evento.category));
                        containerCards.innerHTML = "";
    ...
    search.addEventListener("keyup", (event) => {
        if (eventosCheckeados.length !== 0) {
            ...
            let texto = event.target.value.toLowerCase();
            let tarjetasBuscadas = filtrarTexto(eventosPasados, texto);
            containerCards.innerHTML = "";
            createCards(tarjetasBuscadas);
            if (tarjetasBuscadas.length === 0) {
                containerCards.innerHTML = "<h3>No hay resultados</h3>"
